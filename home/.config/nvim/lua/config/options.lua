...
-- Cursor 的な AI 補完は LazyVim の extras で組んでいる (lazyvim.json):
--   ai.copilot-native: Neovim 組み込みの inline completion で Copilot のゴーストテキストを出し、Tab で確定する
--   ai.sidekick: 編集後に Tab で次の修正箇所へ飛ぶ (Copilot の Next Edit Suggestions) のと、
--                Claude Code / Codex などの CLI を nvim 内で開いて選択範囲やファイルを渡す (<leader>a)
