...
**`docs/keymap.md` が仕様で、設定はその実装。**キーバインドに関わる設定 (xremap、niri、
Hyprland、Karabiner、Magnet、Ghostty の keybind、DMS のバインド) を変える指示を受けたら、
必ず先に `docs/keymap.md` を読んで、その変更が仕様と矛盾しないか確かめる。

- 矛盾しないなら、設定を入れてよい。ただし仕様に無い機能を足すなら、**設定より先に仕様書を
  更新する。**
- 矛盾するなら、設定を入れる前に指摘する。仕様を変えるのか、別のキーにするのかを決めてから
  手を動かす。

順序を守るのは、設定が 5 つのレイヤに散っていて、片方だけ直すと両 OS で挙動がずれるから。
過去に Option 面をウィンドウマネージャに充てて端末の編集操作を全部潰しかけた
(`docs/keymap.md` §2 に経緯がある)。仕様を先に通していれば机上で気づけた。
