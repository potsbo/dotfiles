  ...
  services = {
    nextcloud = {
      ...
      settings = {
        ...
        # Serve が X-Forwarded-For に載せてくる発信元 (tailnet の 100.x) を採る。
        # 無いと remoteAddr が全部 127.0.0.1 になり、IP ごとに効くはずの
        # ブルートフォース抑制がバケツ 1 つに潰れる (他人の失敗がこちらの
        # 正規ログインまで遅延させる)。ログでも誰が叩いたか分からなくなる。
        # この tailnet には他人の端末も居るので、ここは効いていないと困る。
        #
        # 偽装できる者が居ないので loopback を信頼してよい。127.0.0.1:8080 に
        # 到達できるのは Serve だけで、firewall に穴が無い。
